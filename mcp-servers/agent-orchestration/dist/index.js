...
function loadEnvironment() {
  try {
    let envResult = import_dotenv.default.config();
    if (envResult.error) {
      const rootEnvPath = import_path.default.resolve(process.cwd(), "../../.env");
      if (import_fs.default.existsSync(rootEnvPath)) {
        envResult = import_dotenv.default.config({ path: rootEnvPath });
      }
    }
    if (envResult.error) {
...
var TaskQueueSystem = class {
  constructor() {
    ...
    this.queue = new import_bullmq.Queue("agent-orchestration", {
    ...
    this.registerTools([
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          ...
          prompt: import_zod.z.string().describe("The prompt or instructions to send to the agent"),
          brandContext: import_zod.z.string().optional().describe("The brand context to use (optional)"),
          temperature: import_zod.z.number().min(0).max(2).optional().describe("Temperature for generation (optional)"),
          maxTokens: import_zod.z.number().positive().optional().describe("Maximum tokens to generate (optional)")
        }),
        func: async (input) => {
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          ...
          payload: import_zod.z.record(import_zod.z.any()).describe("The task payload (data needed for the task)"),
          brandContext: import_zod.z.string().optional().describe("The brand context (optional)"),
          priority: import_zod.z.enum(["low", "medium", "high", "critical"]).optional().describe("Task priority (optional)"),
          dependsOn: import_zod.z.array(import_zod.z.string()).optional().describe("Task IDs this task depends on (optional)"),
          timeout: import_zod.z.number().positive().optional().describe("Task timeout in milliseconds (optional)")
        }),
        func: async (input) => {
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          taskId: import_zod.z.string().describe("The ID of the task to check")
        }),
        func: async (input) => {
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          taskId: import_zod.z.string().describe("The ID of the task to get results for")
        }),
        func: async (input) => {
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          name: import_zod.z.string().describe("Name of the agent"),
          ...
          capabilities: import_zod.z.array(import_zod.z.enum([
            "content_generation",
            "context_validation",
            "asset_management",
            "analytics",
          ...
          modelProvider: import_zod.z.enum([
            "openai",
            "anthropic",
            "ollama",
            "custom"
          ]).describe("The model provider"),
          modelName: import_zod.z.string().describe("The model name")
        }),
        func: async (input) => {
      ...
      new MCPTool({
        ...
        inputSchema: import_zod.z.object({
          workflowType: import_zod.z.string().describe("Type of workflow to orchestrate"),
          steps: import_zod.z.array(import_zod.z.object({
            ...
            instruction: import_zod.z.string().describe("Instruction for this step"),
            outputKey: import_zod.z.string().describe("Key to store the output under"),
            condition: import_zod.z.string().optional().describe("Condition to execute this step (optional)")
          })).describe("Steps in the workflow"),
          context: import_zod.z.record(import_zod.z.any()).describe("Initial context for the workflow"),
          brandContext: import_zod.z.string().optional().describe("Brand context (optional)"),
          timeout: import_zod.z.number().positive().optional().describe("Workflow timeout in milliseconds (optional)")
        }),
        func: async (input) => {
    ...
    this.registerResources([
      ...
      new MCPResource({
        ...
        schema: import_zod.z.object({
          ...
          agents: import_zod.z.array(import_zod.z.object({
            id: import_zod.z.string(),
            name: import_zod.z.string(),
            type: import_zod.z.string(),
            capabilities: import_zod.z.array(import_zod.z.string()),
            status: import_zod.z.string(),
            modelProvider: import_zod.z.string(),
            modelName: import_zod.z.string(),
            lastActive: import_zod.z.union([import_zod.z.string(), import_zod.z.undefined()])
      ...
      new MCPResource({
        ...
        schema: import_zod.z.object({
          // Added schema
          activeCount: import_zod.z.number(),
          waitingCount: import_zod.z.number(),
          completedCount: import_zod.z.number(),
          failedCount: import_zod.z.number(),
          avgProcessingTime: import_zod.z.number()
        }),
        get: async () => {
          return {
            activeCount: 5,
...
var AgentManager = class {
  ...
  constructor() {
    ...
    if (anthropicApiKey) {
      this.anthropicClient = new import_sdk.default({ apiKey: anthropicApiKey });
    } else {
  ...
  async executeOllama(agent, request) {
    ...
    try {
      const response = await (0, import_node_fetch.default)(`${ollamaBaseUrl}/api/chat`, {
        method: "POST",
        headers: {
